...
if test "x$withval" = xyes && test x$VALID_CUDA = xyes ; then 
	AC_MSG_NOTICE([Building with CUDA bindings])
elif test "x$withval" = xyes && test x$VALID_CUDA = xno ; then 
	AC_MSG_NOTICE([Cannot build CUDA bindings. Check errors])
fi

AM_CONDITIONAL([ENABLE_CUDA], [test x$VALID_CUDA = xyes])
